...
void Weapon_Chainsaw (edict_t *ent)
{
	...
	// 20-27   cut   :  vorgestreckter Angriff als best�ndiger Loop...aber beim stoppen immer noch bis fram 19 laufen lassen!
	// 28-35    cutoff :  S�ge zur�ck in Trageposition
	// 36-55   idle  : rumdragen halt  p�tt-p�tt-p�tt  :)
  ...
  for (n = 0; n < 48; n++)
    {
      ...
      nuke_angs[1] = n * 32;
      nuke_angs[2] = 0;
      AngleVectors (nuke_angs, forward, right, up);
      nuke = G_Spawn();
      VectorCopy (origin, nuke->s.origin);
      VectorClear (nuke->velocity);
      VectorMA (nuke->velocity, 550, forward, nuke->velocity);
...
void fire_nuke (edict_t *self, vec3_t start, vec3_t aimdir, int speed, float timer)
{
  ...
  VectorSet (nuke->avelocity, 300, 300, 300);
