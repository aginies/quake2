...
void SetCTFStats(edict_t *ent, short *stats)
{
    ...
    //gi.dprintf ("DEBUG: SetCTFStats %c \n", tnames[i]);
	while (tnames[i]) {
...
gitem_t *CTFWhat_Tech(edict_t *ent)
{
    ...
    //gi.dprintf ("DEBUG: CTFWhat_Tech %c \n", tnames[i]);
	while (tnames[i]) {
		if ((tech = FindItemByClassname(tnames[i])) != NULL &&
			ent->client->pers.inventory[ITEM_INDEX(tech)]) {
			return tech;
...
qboolean CTFPickup_Tech (edict_t *ent, edict_t *other)
{
    ...
    //gi.dprintf ("DEBUG: CTFPickup_Tech %c \n", tnames[i]);
	while (tnames[i]) {
		if ((tech = FindItemByClassname(tnames[i])) != NULL &&
...
int CTFApplyResistance(edict_t *ent, int dmg)
{
	...
	// DEBUG if (dmg && tech && ent->client && ent->client->pers.inventory[ITEM_INDEX(tech)]) {
...
pmenu_t creditsmenu[] = {
	...
	{ "*Chaos Deathmatch v1.16.2",				PMENU_ALIGN_CENTER, NULL, NULL },
